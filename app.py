...
MISSIONS = [
    ...
    {"title": "대형 편의점 공습", "news": "🍱 유원지 바로 옆 24시간 편의점 오픈! 즉석식품 반값 할인 시작.", "choices": ["[맞불 가격 경쟁] 전 메뉴 가격 20% 인하", "[품질 차별화] 편의점과 차별화된 수제 퀄리티 홍보 (마케팅 2만원)", "[현상 유지] 기존 가격과 영업 방식 유지"], "multiplier": 0.7},
    ...
    {"title": "쓰레기통 대란", "news": "🗑️ 쓰레기통 포화 및 악취 민원 발생! 손님들이 불쾌감을 느낍니다.", "choices": ["[클린 존 마케팅] 트럭 앞 청소 알바 고용 (고정비 2만원 지출)", "[친환경 캠페인] 수거용 친환경 패키지 전환 (원가 10% 상승)", "[현상 유지] 내 트럭 내부만 관리하고 기존 방식 고수"], "multiplier": 0.7},
    ...
    {"title": "스폰서 음료 배포", "news": "🥤 대기업 홍보 부스에서 생수와 캔콜라를 전원 무료 배포합니다!", "choices": ["[단품 집중 할인] 가격 20% 인하 전략", "[사이드 메뉴 강화] 결합 마케팅 (고정비 2만원 지출)", "[현상 유지] 가격 및 마케팅 변동 없이 고수"], "multiplier": 1.0},
    {"title": "학교 단체 소풍", "news": "🎒 근처 중·고등학교에서 학생 500명이 단체 소풍을 왔습니다!", "choices": ["[학생 특별 할인] 가격 15% 인하로 학생층 흡수", "[전투 영업] 빠른 회전율을 위한 임시 알바 고용 (고정비 2만원 지출)", "[현상 유지] 평소 가격과 인원으로 영업"], "multiplier": 1.7},
...
def render_student_view():
    ...
    if st.session_state.my_phase == "대기실":
        ...
        if user_data["night_ready"]:
            ...
            st.write(f"➖ **고정비(자리세/마케팅 등):** {receipt['fixed_cost']:,}원")
            st.info(f"**최종 잔고:** {user_data['cash']:,}원")
            
            st.divider()
            st.subheader("🚚 내일 장사 도매상 발주")
            menu_cost = MENUS[user_data["menu"]]["cost"]
            st.write(f"*(내 메뉴 원가: {menu_cost:,}원 / 1회 물류비: 20,000원)*")
            
            with st.form("order_form"):
                order_qty = st.selectbox("추가 발주 인분 수", [0, 30, 50, 70, 100])
                total_cost = (order_qty * menu_cost) + 20000 if order_qty > 0 else 0
